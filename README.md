...
`WARNING`: This is an experiment in **vibecoding**, not a quality product. This was mostly generated by AI. My aim is to generate conversation around **vibecoding** and point people to this project.
...
- **Debugging** — A debugging API to set breakpoints and step through the code
...
nvm.addProc("add") do (args: seq[Value]) -> Value:
  intValue(args[0].intVal + args[1].intVal)
...
Nimmy has comprehensive tests including:
- **Gold master tests** — Scripts with expected output that is compared to actual output
- **Stepping tests** — Tests for the VM's step-by-step execution
- **Debugging tests** — Tests for breakpoints, step-over, step-into, step-out
- **Interactive tests** — Tests for the interactive console/REPL mode

Run all tests with: `nim r tests/test_all.nim`
